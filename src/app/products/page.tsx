...
// กำหนด Props สำหรับหน้ารายละเอียดสินค้าแต่ละตัว
// params มาในรูปแบบ Promise เพราะใน Next.js เวอร์ชันใหม่เป็น Asynchronous
interface ProductDetailPageProps {
  params: Promise<{
    id: string // ID ของสินค้าที่ดึงมาจาก URL เช่น /products/1
  }>
}

// Dynamic Metadata: ส่วนที่ทำให้ Google Search หาเราเจอ (SEO)
// ฟังก์ชันนี้รันบน Server เท่านั้น
...
export default async function ProductsPage({ searchParams }: ProductsPageProps) {
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      ...
      {/* ฟอร์มค้นหาสินค้า */}
      {/* เมื่อผู้ใช้พิมพ์และกด Enter หรือกดปุ่มค้นหา จะส่ง Query Parameter ?search=... ไปที่ URL */}
      <form className="mb-8">
        <div className="flex gap-2">
          <input
            type="text"
            name="search" // จะกลายเป็น ?search=value ใน URL
            defaultValue={search || ''} // ใช้ defaultValue เพื่อให้ค่าคงอยู่หลัง Search
            placeholder="ค้นหาสินค้าที่ต้องการ..."
            className="flex-1 p-3 text-lg border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all shadow-sm"
          />
          {/* ปุ่มค้นหา */}
          <button
            type="submit"
            className="px-6 py-3 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 active:scale-95 transition-all shadow-sm hover:shadow-md"
          >
            ค้นหา
          </button>
        </div>
